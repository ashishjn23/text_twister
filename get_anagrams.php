...
function validate($inputword, $rack){
    ...
    $dbhandle = new PDO("sqlite:scrabble.sqlite") or die("Failed to open DB");
    if (!$dbhandle) die ($error);
    $query3 = "select words, ctr3, ctr4, ctr5, ctr6, ctr7 from word where rack_id = '$rack'";
    $statement = $dbhandle->prepare($query3);
    $statement->execute();
    $results = $statement->fetchAll(PDO::FETCH_ASSOC);
    if($results != NULL) {
        if($results[0]["ctr" . strlen($inputword)] == 0){
            $flag = 0;
            $res = "There are no " . strlen($inputword) . " lettered words remaining in the game. Try other words!";
            $ctr3 = $results[0][ctr3];
            $ctr4 = $results[0][ctr4];
            $ctr5 = $results[0][ctr5];
            $ctr6 = $results[0][ctr6];
